...
!
    ...
    DO I = 1, HcoState%NX
       ...
       ! Isoprene [kg/m2/s]
       ! ❗️Use a new subroutine, GET_MEGAN_EMISSIONS_ISOP
       CALL GET_MEGAN_EMISSIONS_ISOP( HcoState, ExtState, Inst, &
...
!BOP
!
! !IROUTINE: Get_Megan_Emissions_forIsoprene
!
! !DESCRIPTION: Subroutine Get\_Megan\_Emissions\_ISOP computes biogenic emissions in
!  units of kg/m2/s using the MEGAN inventory. (dbm, 12/2012)
! !With modified algorithm and forcings for boreal shrub only. (modification indicated)
!  with ❗️
!
!\\
!\\
! !INTERFACE:
!
  SUBROUTINE GET_MEGAN_EMISSIONS_ISOP( HcoState, ExtState, &
                                       Inst, I, J, CMPD, MEGAN_EMIS, RC )
!
! !INPUT PARAMETERS:
!
...
!
! !LOCAL VARIABLES:
! !❗️Add a new var PFTBDS as an indicator of changing forcing, params and algorithm. 
!
    REAL(hp)            :: GAMMA_LAI
    REAL(hp)            :: GAMMA_AGE
    REAL(hp)            :: GAMMA_PAR
    ...
    REAL(hp)            :: ANEW, AGRO, AMAT, AOLD
    REAL(hp)            :: ISOLAI, PMISOLAI, MISOLAI
    REAL(hp)            :: PFTSUM, PFTBDS 
    REAL(hp)            :: CONV_FAC    ! conversion of standard gamma between two versions of algorithm
    LOGICAL             :: BIDIR
    CHARACTER(LEN=255)  :: LOC
    ...
    ! GET_MEGAN_EMISSIONS begins here!
    !=================================================================
    LOC = 'GET_MEGAN_EMISSIONS_ISOP_2024 (HCOX_MEGAN_MOD.F90)'    
    ...
    IF ( ISOLAI * AEF > 0.0_hp ) THEN
       ...
       ! GAMMA_T_LD (temperature activity factor for
       ! light-dependent fraction)
       ! ❗️If BDS is dominantPFT, change forcing to skin temperature [K]
       ! use GET_GAMMA_T_LD_ISOP alternatively 
       ! --------------------------------------------------

       IF ( PFTBDS / PFTSUM >= 0.5_hp ) THEN

          TS = ExtState%TSKIN%Arr%Val(I,J)

          GAMMA_T_LD = GET_GAMMA_T_LD_ISOP( TS, Inst%T_LASTXDAYS(I,J), &
                                         Inst%T_LAST24H(I,J), CT1, CEO )
          CONV_FAC   = 1.0_hp / GET_GAMMA_T_LD_ISOP( 303.0_hp, 297.0_sp, &
                                                     297.0_sp, CT1, CEO )
    ...
    MEGAN_EMIS = Inst%NORM_FAC(1) * AEF * GAMMA_AGE * GAMMA_SM * &
                 GAMMA_LAI * ((1.0_hp - LDF) * GAMMA_T_LI +      &
                 (LDF * GAMMA_PAR * GAMMA_T_LD * CONV_FAC)) * GAMMA_CO2

    ! Leave w/ success
    RC = HCO_SUCCESS

  END SUBROUTINE GET_MEGAN_EMISSIONS_ISOP
!EOC
!------------------------------------------------------------------------------
!                   Harmonized Emissions Component (HEMCO)                    !
!------------------------------------------------------------------------------
...
!BOP
!
! !IROUTINE: Get_Gamma_T_LD_forIsoprene
!
! !DESCRIPTION: Function Get\_Gamma\_T\_LD computes the temperature
!  sensitivity for the light-dependent fraction of emissions.
! !❗️with modified algorithm, and params (Topt, CT2) for BDS ISOP emission
!\\
!\\
! !INTERFACE:
!
  FUNCTION GET_GAMMA_T_LD_ISOP( T, PT_15, PT_1, CT1, CEO ) &
       RESULT( GAMMA_T_LD )
!
! !INPUT PARAMETERS:
!
    ! Current leaf temperature [K], use surface skin temperature instead [TSKIN]
    REAL(hp), INTENT(IN) :: T
...
!
    ...
    !=================================================================
    ! GET_GAMMA_T_LD_ISOP begins here!
    !=================================================================
    !❗️Change made according to Finse site observation and Guenther 2012 Eqn.10 
    E_OPT = CEO * EXP( 0.05_hp * ( PT_15  - 2.97e2_hp ) ) &
                * EXP( 0.05_hp * ( PT_1   - 2.97e2_hp ) )
    T_OPT    = 3.23e2_hp + ( 6.0e-1_hp * ( PT_15 - 2.97e2_hp ) )
    CT1_BDS  = 98.0_hp
    CT2      = 168.0_hp
    ...
    ! effect of average temperature over previous 15 days, based on
    ! Eq 5a, 5b, 5c from Guenther et al, 1999.
    C_T   = E_OPT * CT2     * EXP( CT1_BDS * X ) / &
            ( CT2 - CT1_BDS * ( 1.0_hp - EXP( CT2 * X ) ) )

    ! Hourly emission activity = C_T
    ! Prevent negative values
    GAMMA_T_LD = MAX( C_T, 0.0_hp )

  END FUNCTION GET_GAMMA_T_LD_ISOP
!EOC
!------------------------------------------------------------------------------
!                   Harmonized Emissions Component (HEMCO)                    !
!------------------------------------------------------------------------------
...
!
    ...
    ExtState%TSKIN%DoUse     = .TRUE.     ! ❗️Enable skin temperature [K]
